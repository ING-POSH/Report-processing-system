...
import {
  ...
  List, Modal, message, Empty, Tooltip, Select
} from 'antd'
import {
  AudioOutlined, FileTextOutlined, CheckCircleOutlined,
...
const STATUS_STEPS = { pending: 0, transcribing: 1, generating: 2, completed: 3 }
...
export default function PartnerTranscriber() {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (['completed', 'failed'].includes(job.status)) {
          ...
          setPollingId(null)
          if (job.status === 'completed') message.success('Transcription complete!')
  ...
  const handleUpload = async ({ file }) => {
    if (!spaceId) { message.error('Partner space not found'); return }
    setUploading(true)
    try {
      ...
      formData.append('report_format', 'partner_generic')
  ...
  return (
    <div>
      ...
      <Text type="secondary" style={{ display: 'block', marginBottom: 20 }}>
        Upload audio recordings for transcription. Reports are generated in a generic format configurable per project.
      </Text>

      <Card bordered={false} style={styles.card}
        title={<Space><AudioOutlined style={{ color: '#d4a017' }} /><span>Upload Audio File</span></Space>}>

        <Alert
          type="warning"
          showIcon
          message="Partner Generic Mode"
          description="Reports generated here use a configurable generic template — separate from UN-Habitat internal reporting standards."
          style={{ marginBottom: 16 }}
        />

        <div style={{ marginBottom: 16 }}>
          <Text style={{ display: 'block', marginBottom: 6, fontSize: 13 }}>Link to Project (optional)</Text>
          <Select
        ...
        <Upload.Dragger
          ...
          style={{ borderColor: '#d4a017', borderRadius: 8 }}
        >
          <p className="ant-upload-drag-icon">
            <AudioOutlined style={{ fontSize: 36, color: '#d4a017' }} />
          </p>
        ...
        {(uploading || pollingId) && (
          <div style={{ marginTop: 20 }}>
            <Steps
              ...
              size="small"
              items={[
                { title: 'Upload', description: 'Received' },
                { title: 'Transcribe', description: 'Speech to text' },
                { title: 'Generate', description: 'Generic format' },
                { title: 'Done', description: 'Ready' },
              ]}
            />
      ...
      <Card bordered={false} style={{ ...styles.card, marginTop: 16 }} title="Previous Transcriptions">
        {loadingJobs ? (
                ...
                <List.Item actions={[
                  job.status === 'completed' && (
                    <Tooltip title="View Report">
                      <Button type="text" icon={<EyeOutlined />} size="small" onClick={() => setViewJob(job)} />
                    </Tooltip>
                  )
                ].filter(Boolean)}>
                  <List.Item.Meta
                    title={
                      <Space>
                        ...
                        <Tag color="orange" style={{ fontSize: 11 }}>Generic Format</Tag>
                      </Space>
                    }
                    description={<Text type="secondary" style={{ fontSize: 12 }}>Submitted {new Date(job.created_at).toLocaleString()}</Text>}
                  />
                </List.Item>
              )
            }}
      ...
      <Modal
        ...
        footer={[
          <Button key="copy" icon={<CopyOutlined />} onClick={() => copyToClipboard(viewJob?.generated_report || '')}>Copy Report</Button>,
          <Button key="transcript" onClick={() => copyToClipboard(viewJob?.transcript_text || '')}>Copy Transcript</Button>,
          <Button key="close" type="primary" onClick={() => setViewJob(null)} style={{ backgroundColor: '#d4a017', borderColor: '#d4a017' }}>Close</Button>
        ]}
        width={700}
      >
        {viewJob && (
          <div>
            <Tag color="orange" style={{ marginBottom: 12 }}>Partner Generic</Tag>
            ...
            <Card size="small" style={{ background: '#fafafa', marginBottom: 16, maxHeight: 200, overflow: 'auto' }}>
              <Paragraph style={{ fontSize: 13, whiteSpace: 'pre-wrap', margin: 0 }}>{viewJob.transcript_text || 'No transcript'}</Paragraph>
            </Card>
            <Divider orientation="left">Generated Report</Divider>
            <Card size="small" style={{ background: '#fffbe6', maxHeight: 350, overflow: 'auto' }}>
              <Paragraph style={{ fontSize: 13, whiteSpace: 'pre-wrap', margin: 0 }}>{viewJob.generated_report || 'No report generated'}</Paragraph>
            </Card>
          </div>
        )}
      </Modal>
